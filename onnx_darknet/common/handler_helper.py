...
from onnx_darknet.handlers.backend import *  # noqa
from onnx_darknet.handlers.backend_handler import BackendHandler
from onnx_darknet.handlers.frontend import *  # noqa
...
def get_all_frontend_handlers(opset_dict):
  ...
  for handler in FrontendHandler.__subclasses__():
    ...
    for darknet_op in handler.DARKNET_OP:
      handlers.setdefault(domain, {})[darknet_op] = handler
  return handlers
...
def get_frontend_coverage():
  ...
  darknet_coverage: e.g. {'domain': {'DARKNET_OP': [versions], ...}, ...}
  """
  ...
  for handler in FrontendHandler.__subclasses__():
    ...
    domain = handler.DOMAIN
    for darknet_op in handler.DARKNET_OP:
      _update_coverage(darknet_coverage, domain, op_name_to_lower(darknet_op), versions)
    _update_coverage(onnx_coverage, domain, handler.ONNX_OP, versions)
  return onnx_coverage, darknet_coverage
