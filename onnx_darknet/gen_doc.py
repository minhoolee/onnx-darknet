...
from onnx_darknet import opset_version
import onnx_darknet.backend
import onnx_darknet.backend_rep
import onnx_darknet.frontend
from third_party import get_info


def main(docs_dir):
  gen_doc_for = {
      'onnx_darknet.backend': [
          onnx_darknet.backend.prepare,
      ],
      'onnx_darknet.frontend': [
          onnx_darknet.frontend.tensorflow_graph_to_onnx_model,
      ],
      'onnx_darknet.backend_rep.DarknetRep': [
  ...
  with open(os.path.join(docs_dir, 'support_status.md'), 'w') as status_file:
    ...
    status_file.write('| -------------- |:------------------:|\n')
    for key, val in sorted(opset_version.frontend_darknet_opset_version.items()):
      version_str = str(val)[1:-1]
      status_file.write("|{}|{}|\n".format(key, version_str
                                           if len(version_str) else "N/A"))
